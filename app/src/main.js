...
function showSuccess(user, message, color) {
    ...
    if (successAlert) {
        ...
        var alertDiv = successAlert.closest(".alert.success"); // Find the parent alert div
        if (alertDiv) {
            alertDiv.style.background = color; // Set background color
            alertDiv.style.opacity = "100";
            alertDiv.style.display = "block";
            setTimeout(function(){ alertDiv.style.display = "none"; }, 5000); // Hide after 5 seconds
        }
    }
}
...
const App = {
   ...
   register: async function(user) {
    ...
    // ✅ Check if consent checkbox is checked for Donor or Patient
    if ((user === "Donor" && !document.getElementById("DonorConsent").checked) ||
        (user === "Patient" && !document.getElementById("PatientConsent").checked)) {
    ...
    if (checkedValues) {
        ...
        if (!validate) {
            ...
            try {
                ...
                showWarning(user, "Transaction failed: " + error.message, "#f44336");
            }
        }
        else {
...
// Random fact logic is now handled in translations.js
